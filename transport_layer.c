...
struct tcpheader {
 uint16_t srcport;              // 16 bit source port 
 uint16_t destport;            // 16 bit destination port 
 unsigned int seqnum;         //  8 bit sequence no.
 unsigned int acknum;        //   8 bit acknowledgement no.
...
void send_init(unsigned char *add) // called first 
{
...
void receive_init()    // second function called is this
{
...
void receiver()
{
    ...
    {
        printf("TCP Headercsum((unsigned short*)buffer, Error - CheckSum..!!\n");
        exit(-1);
    }
...
int main(int argc, char * argv[])
{
    ...
    if(argv[1][0]=='1') send_init(argv[2]);
        ...
        printf("Wrong Agruments.!!\nAborting....!!\n");
    exit(-1);
}
    printf("Transport Layer test has been established !!\n");
    return 0;
}